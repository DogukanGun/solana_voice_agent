...
export async function openbookCreateMarket(
  ...
  const raydium = await Raydium.load({
    owner: agent.isUiMode ? agent.wallet_address : agent.wallet,
  ...
  const { execute, transactions } = await raydium.marketV2.create({
    baseInfo: {
      mint: baseMint,
  ...
  if(agent.isUiMode){
    const txHashs = await agent.signMultipleTransactions(transactions.map((tx)=>Buffer.from(tx.serialize()).toString("base64")));
    return txHashs;
  }
