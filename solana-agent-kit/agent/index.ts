...
export class SolanaAgentKit {
  ...
  public wallet: Keypair | undefined;
  public wallet_address: PublicKey;
  public config: Config;
  public isUiMode: boolean = false;
  public onSignTransaction: ((tx: string) => Promise<string>) | null = null;
  ...
  constructor(
    ...
    if(this.isValidPublicKey(private_key)) {
  ...
  signByUser = async (tx:string):Promise<string>=> {
    if(this.onSignTransaction === null) {
      throw new Error("onSignTransaction callback is not set")
    }
    return await this.onSignTransaction(tx);
  }

  signMultipleTransactions = async (txs: string[]):Promise<string[]> => {
    if(this.onSignTransaction === null) {
      throw new Error("onSignTransaction callback is not set")
    }
    let signatures = [];
    for(const tx of txs) {
      signatures.push(await this.onSignTransaction(tx));
    }
    return signatures;
  }
